...
# 2. 将本地 bin 目录加入路径 (Local bin path)
if [ -d "$HOME/.local/bin" ] ; then
    export PATH="$HOME/.local/bin:$PATH"
fi

# 3. 加载 NVM (Load NVM - 解决 Node.js 问题的核心)
...
# 加载本地环境变量
[[ -f ~/.api-key.local ]] && source ~/.api-key.local


# >>> conda initialize >>>
# !! Contents within this block are managed by 'conda init' !!
__conda_setup="$('/home/lr/miniconda3/bin/conda' 'shell.zsh' 'hook' 2> /dev/null)"
if [ $? -eq 0 ]; then
    eval "$__conda_setup"
else
    if [ -f "/home/lr/miniconda3/etc/profile.d/conda.sh" ]; then
        . "/home/lr/miniconda3/etc/profile.d/conda.sh"
    else
        export PATH="/home/lr/miniconda3/bin:$PATH"
    fi
fi
unset __conda_setup
# <<< conda initialize <<<
...
export MAMBA_EXE='/home/lr/miniconda3/bin/mamba';
export MAMBA_ROOT_PREFIX='/extra_storage/mamba';
